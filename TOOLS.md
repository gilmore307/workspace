...
- helpers should use registry config ids for automation; do not add key-input secret helper APIs. Secret resolution order is: registry config id → config payload secret alias → `/root/secrets/registry.json` entry → local secret file
